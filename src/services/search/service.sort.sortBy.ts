...
const readPostsSortByService = async (
    lang: string,
    q: string,
    page: number = 0 ,
    ...
    try {
        let query =
            ...
            if (sort_by === 'asc' || sort_by === 'ASC') {
                query += `ORDER BY posts.created_at ASC LIMIT 20 OFFSET ${page * 20}`;
            } else {
                query += `ORDER BY posts.created_at DESC LIMIT 20 OFFSET ${page * 20}`;
            }
...
export default readPostsSortByService;
