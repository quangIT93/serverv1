...
const readPostsSalarySortService = async (
    lang: string,
    q: string,
    page: number = 0 ,
    ...
    try {
        let query =
            ...
            if (salary_sort === 'asc' || salary_sort === 'ASC') {
                query += `ORDER BY posts.salary_max ASC LIMIT 20 OFFSET ${page * 20}`;
            } else {
                query += `ORDER BY posts.salary_max DESC LIMIT 20 OFFSET ${page * 20}`;
            }
...
export default readPostsSalarySortService;
