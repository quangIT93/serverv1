...
const readApplicationCategoriesByIdService = async (applicationId: number) => {
    try {
        ...
        const query = "SELECT applications_categories.category_id as child_category_id, child_categories.name as child_category," +
        " child_categories.parent_category_id as parent_category_id, parent_categories.name as parent_category" +
