...
const createApplicationExperiencesService = async (applicationId: number, accountId: string) => {
    try {
        logging.info("Create application EXPERIENCES service start ...");
        const query = "INSERT INTO applications_experiences (application_id, title, company_name, start_date, end_date, extra_information)" +
        ...
        logging.error(
            "Create application EXPERIENCES service has error: ",
            error
        );
        return null;
    }
