 ...
 * @returns
 *
 * This query is used to get all users chatted with the current user
 *
 */

const getUsersChattedV2 = async (lang: string, id: string) => {
  try {
    ...
    const query = `
        ...
        FROM (
            ...
            LEFT JOIN posts ON posts.id = t.post_id
            LEFT JOIN profiles ON profiles.id = IF (
                    t.sender_id = ?,
                    t.receiver_id,
                    t.sender_id
                )
            LEFT JOIN salary_types ON posts.salary_type = salary_types.id
            LEFT JOIN post_images ON posts.id = post_images.post_id
            LEFT JOIN applications ON posts.id = applications.post_id
            AND applications.account_id = ?
        GROUP BY t.id
...
export default getUsersChattedV2;
