...
async function formatPostBeforeReturn(post: PostService) {
  const postResponse: PostResponse = {
    ...
    share_link: `${process.env.WEB_DEEP_LINK}?link=${
      process.env.PREVIEW_PAGE_URL
    }?postId=${
      post.post_id ? post.post_id : post.id
    }&st=${`${encodeURIComponent(post.title)}`}&sd=${
      ""
    }&si=`,
  };
  if (post.image !== undefined && post.image !== null) {
    postResponse.image =
      `${process.env.AWS_BUCKET_IMAGE_URL}/${ImageBucket.POST_IMAGES}/${
        post.post_id ? post.post_id : postResponse.id
      }/` + post.image;
    } else {
    ...
    postResponse.share_link += postResponse.image ? postResponse.image : "";
