...
const createPostController = async (
  ...
  try {
    ...
    multerUploadImages(req, res, async (err) => {
      ...
      const companyResourceId = req.body.companyResourceId
        ? req.body.companyResourceId
        : null;

      const jobTypeId = req.body.jobTypeId ? req.body.jobTypeId : null;

      const expiredDate = Number.isInteger(+req.body.expiredDate)
      ...
      if (phoneNumber && !helper.checkPhoneNumberFormat(phoneNumber)) {
        return next(
          createError(400, "Please enter a valid Vietnamese phone number")
        );
      }
      ...
      if (description.length > 4000) {
        logging.warning("Invalid description");
        return next(
          createError(400, "Description must be less than 4000 characters")
        );
      }
      ...
      const isCreateCategoriesOfPostSuccess = await postCategoryServices.create(
        postIdCreated,
        categoryIds
      );
      ...
      if (companyResourceId && siteUrl && (role === 2 || role === 1)) {
        const isCreatePostResourceSuccess = await createPostResourceService(
          postIdCreated,
          siteUrl,
          +companyResourceId
        );
