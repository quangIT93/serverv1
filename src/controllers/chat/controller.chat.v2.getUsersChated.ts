...
const getUsersChattedV2Controller = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    ...
    const id = req.user.id;
        ...
        // Get users chatted
        const usersChatted = await chatServices.getUsersChattedV2(
    ...
    await Promise.all(
            usersChatted.map(async (userChatted, _) => {
                ...
                if (userChatted.name === null) {
                    userChatted.name = userChatted.post_title || "Anonymous";
                }
...
export default getUsersChattedV2Controller;
