...
const createRateForApplicationController = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        // CREATE RATE
        const rateResult = await applicationService.create.createRateForApplication(+applicationId, +rate, comment);

        if (!rateResult) {
            logging.warning('Create rate for application controller failed');
            return next(createError(500, 'Create rate for application controller failed'));
        }

        applicationService.update.updateById(+applicationId, 6);
